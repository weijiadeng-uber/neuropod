                                                                                       ...
                                                                                       jstring path)
{
    try
    {
        (dlopen((detail::CWD + "/neuropod/backends/tensorflow/libneuropod_tensorflow_backend.so").c_str(),
                RTLD_NOW | RTLD_GLOBAL));
        auto                convertedPath = toString(env, path);
        neuropod::Neuropod *ret           = nullptr;
        if (isTestMode) {
            ret = new neuropod::Neuropod(convertedPath, detail::ope_backend_location_overrides);
        } else {
...
ret           = new neuropod::Neuropod(convertedPath, *opts);
    ...
    {
        throwJavaException(env, detail::CWD.c_str());
    }
    return reinterpret_cast<jlong>(nullptr);
}
